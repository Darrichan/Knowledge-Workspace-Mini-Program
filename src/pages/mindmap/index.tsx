...
const textUnits = (value: string) => Array.from(value || '').reduce((total, char) => total + (/[^\x00-\xff]/.test(char) ? 1 : .58), 0)
const nodeWidth = (label: string, fontSize = 28, root = false) => {
...
export default function MindMapPage() {
  ...
  const [document, setDocument] = useState<DocumentItem | null>(null); const [, setMap] = useState<MindMapItem | null>(null)
  ...
  const widestNode = Math.max(nodeWidth(rootLabel, 38, true), ...nodes.map(node => nodeWidth(node.label, node.fontSize || 28)), 260)
  const boardWidth = Math.max(1450, 690 + maxDepth * (widestNode + 135))
  ...
  const beginNodeEdit = (nodeId: string) => { setSelectedId(nodeId); setEditingId(nodeId) }
  const updateEditingLabel = (value: string) => { if (editingId === rootId || editingId === 'root') setRootLabel(value); else if (editingId) updateNode(editingId, { label: value }) }
  const addNode = (asChild: boolean) => { const parent = asChild ? selectedId : selected?.parent_id || rootId; const node: MindMapNode = { id: `${Date.now()}`, label: '新主题', color: selected?.color || COLORS[nodes.length % COLORS.length], parent_id: parent, fontSize: 28 }; setNodes(current => [...current, node]); setSelectedId(node.id); setEditingId(node.id) }
  ...
  const deleteSelected = async () => { if (selectedId === rootId || selectedId === 'root') return Taro.showToast({ title: '中心主题不能删除', icon: 'none' }); const answer = await Taro.showModal({ title: '删除主题', content: '子主题也会一起删除。' }); if (!answer.confirm) return; const ids = new Set([selectedId]); let changed = true; while (changed) { changed = false; nodes.forEach(node => { if (node.parent_id && ids.has(node.parent_id) && !ids.has(node.id)) { ids.add(node.id); changed = true } }) } setNodes(current => current.filter(node => !ids.has(node.id))); setSelectedId(rootId); setEditingId(null) }
  ...
  const editingValue = editingId === rootId || editingId === 'root' ? rootLabel : nodes.find(node => node.id === editingId)?.label || ''
  ...
  return <View className='mind-page'><View className='mind-toolbar'><Text className='mind-toolbar__status'>{status}</Text><ScrollView className='mind-toolbar__actions' scrollX showScrollbar={false}><View className='mind-toolbar__actions-inner'><View onClick={() => addNode(false)}>＋ 同级</View><View onClick={() => addNode(true)}>＋ 子主题</View><View onClick={openHistory}>历史</View><View className='danger' onClick={deleteSelected}>删除</View></View></ScrollView></View>
    {editingId && <View className='mind-node-editor'><Input focus value={editingValue} maxlength={300} confirmType='done' onInput={event => updateEditingLabel(event.detail.value)} onConfirm={() => setEditingId(null)} /><View onClick={() => setEditingId(null)}>完成</View></View>}
    <ScrollView className='mind-canvas' scrollX scrollY enhanced showScrollbar={false} onTouchStart={onCanvasTouchStart} onTouchMove={onCanvasTouchMove} onTouchEnd={onCanvasTouchEnd} onTouchCancel={onCanvasTouchEnd}><View className='mind-stage' style={{ width: `${boardWidth * zoom}px`, height: `${boardHeight * zoom}px` }}><View className='mind-board' style={{ width: `${boardWidth}px`, minHeight: `${boardHeight}px`, transform: `scale(${zoom})` }}><View className={`root-node ${selectedId === rootId ? 'selected' : ''}`} style={{ width: `${nodeWidth(rootLabel, 38, true)}px` }} onClick={() => beginNodeEdit(rootId)}><Text>{rootLabel || '中心主题'}</Text></View><View className='mind-trunk' /><View className='branch-stack'>{ordered.map(node => <View className='branch-row' key={node.id} style={{ marginLeft: `${Math.max(0, depth(node) - 1) * 115}px` }}><View className='branch-line' style={{ borderColor: node.color }} /><View className={`branch-node ${selectedId === node.id ? 'selected' : ''}`} style={{ borderBottomColor: node.color, width: `${nodeWidth(node.label, node.fontSize || 28)}px` }} onClick={() => beginNodeEdit(node.id)}>{node.priority ? <Text className='node-priority' style={{ background: COLORS[(node.priority - 1) % COLORS.length] }}>{node.priority}</Text> : null}{node.marker ? <Text className='node-marker'>{node.marker}</Text> : null}<Text className='branch-node__label' style={{ fontSize: `${node.fontSize || 28}px` }}>{node.label || '新主题'}</Text></View></View>)}{!nodes.length && <View className='empty-branch' onClick={() => addNode(true)}><Text>＋</Text><Text>添加第一个分支</Text></View>}</View></View></View></ScrollView>
